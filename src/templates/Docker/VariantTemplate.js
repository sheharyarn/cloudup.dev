...
const prepareAllFiles = (files, configVars, userVars) =>
  Object.keys(files).reduce((preparedFiles, filetype) => {
    preparedFiles[filetype] = prepareOneFile(
      files[filetype],
      configVars,
      userVars
    );
...
const VariantTemplate = ({ data, location, pageContext }) => {
  ...
  const githubIssues =
    URLs.github.repo(github.user, github.repo) + '/issues/new';
      ...
      <div className={styles.container}>
        ...
        <div className={styles.columnInfo}>
          ...
          <div className={styles.opensource}>
            ...
            <p>
              You can also help by reporting bugs or sharing your configuration
              on <ExternalLink url={githubIssues}>GitHub</ExternalLink>.
            </p>
        ...
        <div className={`has-code-overrides ${styles.columnFiles}`}>
          {variant.files.map((ft) => (
            <FileView key={ft} type={ft} text={preparedFiles[ft]} />
          ))}
        </div>
      </div>
    </Layout>
...
const Banner = ({ platform, variant }) => (
  <div className={styles.banner}>
    ...
    <hr />
    <p>
      Get started with Docker for your {platform.name} project with optimized
      and production-ready configs below
    </p>
  </div>
);
...
export const pageQuery = graphql`
  query DockerConfigById($platformId: String!) {
    ...
    platform: yaml(
      fields: { tool: { eq: "docker" }, platformId: { eq: $platformId } }
    ) {
      name
      variants {
        id
